...
func makeCert(caCert *x509.Certificate, caKey *rsa.PrivateKey, subject *pkix.Name, name string, dns string) error {
...
func main() {
	...
	if err := makeCert(caCert, caKey, &subject, "server", "server.egress-server.svc.cluster.local"); err != nil {
		log.Fatal("make Server Certificate error!")
	}
	log.Println("Create and Sign the Server certificate successfully.")

	subject.CommonName = "Middle"
	subject.Organization = []string{"Middle Company"}
	if err := makeCert(caCert, caKey, &subject, "middle", "middle.egress-middle.svc.cluster.local"); err != nil {
		log.Fatal("make Middle Certificate error!")
	}
	log.Println("Create and Sign the Middle certificate successfully.")

	subject.CommonName = "Client"
	subject.Organization = []string{"Client Company"}
	if err := makeCert(caCert, caKey, &subject, "client", "client.egress-client.svc.cluster.local"); err != nil {
		log.Fatal("make Client Certificate error!")
	}
	log.Println("Create and Sign the Client certificate successfully.")

	subject.CommonName = "Nginx"
	subject.Organization = []string{"Nginx Company"}
	if err := makeCert(caCert, caKey, &subject, "nginx", "ingress-nginx-controller.ingress-nginx"); err != nil {
		log.Fatal("make Nginx Certificate error!")
	}
	log.Println("Create and Sign the Nginx certificate successfully.")
}
